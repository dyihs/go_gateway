...
func (t *AccessControl) Find(ctx *gin.Context, tx *gorm.DB, search *AccessControl) (*AccessControl, error) {
	model := &AccessControl{}
	err := tx.WithContext(ctx).Where(search).Find(model).Error
	return model, err
}

func (t *AccessControl) Save(ctx *gin.Context, tx *gorm.DB) error {
